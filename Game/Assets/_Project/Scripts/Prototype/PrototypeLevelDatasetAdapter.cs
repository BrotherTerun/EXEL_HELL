...
{
    /// <summary>
    /// Playtest-only bridge that swaps the graybox dataset after a level instance is created.
    /// Keeps both timing branches on identical level data without rewriting the frozen core.
    /// </summary>
    public sealed class PrototypeLevelDatasetAdapter : MonoBehaviour
    {
        ...
        private static double Expected(string goalStringId, PrototypeLevelDataset dataset)
        {
            ...
            {
                case "goal.salary":
                    return dataset.Salary.Sum();
                case "goal.overtime":
                    return dataset.Overtime.Sum();
                case "goal.bonus":
                    return dataset.Bonus.Sum();
                case "goal.bonus5":
                    return dataset.Bonus.Where(value => value >= 5d).Sum();
