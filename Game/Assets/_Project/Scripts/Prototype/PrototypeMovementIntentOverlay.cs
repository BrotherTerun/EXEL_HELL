...
{
    /// <summary>
    /// Presentation-only adapter for the active #REF! movement intent.
    /// It never changes anomaly state or spawn scheduling; it only mirrors currentIntent
    /// as a translucent overlay so active spread and future outbreak telegraphs can coexist.
    /// </summary>
    public sealed class PrototypeMovementIntentOverlay : MonoBehaviour
    {
        ...
        {
            if (FindFirstObjectByType<PrototypeMovementIntentOverlay>() != null)
                return;

            new GameObject("EXEL HELL Movement Intent Overlay").AddComponent<PrototypeMovementIntentOverlay>();
        }
        ...
        private void Bind(ExcelHellPrototype owner)
        {
            ...
            foreach (var view in owner.GetComponentsInChildren<ExcelHellCellView>(true))
            {
                ...
                if (!cellName.StartsWith("Cell ") || cellName.Length <= 5)
                    continue;
